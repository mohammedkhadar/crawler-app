...
import { URL, BrokenLink } from "../types";
...
const URLDetailView: React.FC<URLDetailViewProps> = ({ url, onClose }) => {
  ...
  const fetchBrokenLinks = async (): Promise<void> => {
    try {
  ...
  const createChart = (): void => {
    const canvas = document.getElementById("linksChart") as HTMLCanvasElement;
    if (!canvas) return;

    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    const total = (url.internal_links || 0) + (url.external_links || 0);
    ...
    const internalAngle = ((url.internal_links || 0) / total) * 2 * Math.PI;
    const externalAngle = ((url.external_links || 0) / total) * 2 * Math.PI;
    ...
    if ((url.internal_links || 0) > 0) {
      ...
      ctx.fillText(`${url.internal_links || 0}`, internalLabelX, internalLabelY);
    }

    if ((url.external_links || 0) > 0) {
      ...
      ctx.fillText(`${url.external_links || 0}`, externalLabelX, externalLabelY);
    }
  };
